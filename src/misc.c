...
/* Draw one "brick" on game well */
void set_block(int x,int y,int color, PlrState* plr, int next)
{
	//if(plr->game_area != NULL)
	gdk_draw_pixmap((!next ? plr->game_area->window : plr->next_block_area->window),
		(!next ? plr->game_area->style->black_gc : plr->next_block_area->style->black_gc),
		blocks_pixmap,
		color*BLOCK_WIDTH,0,
